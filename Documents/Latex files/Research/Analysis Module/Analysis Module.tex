...
\begin{document}
    ...
    \section{Support for $C$ and $L$}
    Adding support for reactive elements, the algorithm needs to be changed. Initially the $Analysis$ module supports only resistors,
    meaning the data type used to store values are $float$. In order to support elements like capacitors and inductors, complex 
    impedance needs to replace the $float$ data type. \par
    \begin{center}
        Capacitor: $\frac{1}{j\omega C}$ \par
        Inductor: $j\omega L$ \par
    \end{center}
